...
- 卡片注册在**两个**席位（同一组件、各挂一个实例；宿主 settings 页 `renderSlot("settings.section", …, { only: active })` 同时只挂载一个 section，故两处不会并存、草稿不会分叉）：`settings.models.footer`（list 席位，`id` = 配置 NS，「模型」选项卡底部，与提供方列表同页）与 `settings.plugin.item`（**keyed** 席位，`key` = 配置 NS，「插件」→「插件配置」选项卡，与终端 / Agent 循环 / Subagent / 网页搜索等官方卡并列）。keyed 席位的单元格标识是 `key` 而非 `id`/`order`，两席位不同域，**无需加后缀区分**。
- `settings.plugin.item` 只分发「宿主已服务的命名空间 ∩ 已注册卡的 key」（`ConfigurablePluginsTabController`），故卡在插件配置页出现的前提是 Node 半 `installSection` 成功；该 tab 把卡渲在 `<ul>` 内 ⇒ 根元素须为 `li`（`Card` 的 `as` prop，官方 `PluginCard` 即 `<li>` + `list-style:none`）。
- **两席位都排到各自列表最前，但排序主键不同**（宿主对 `priority`/`order` 一律"越小越靠前"，官方卡都是默认 0 ⇒ 两处都取 `-999999`，给同样想抢首位的第三方插件留余量）：
  - `settings.plugin.item`（keyed）：tab 直接按 `slots.entries()` 的账本序渲染，keyed/单格席位的账本**只按 `priority` 升序稳定排序**（`order` 在该 kind 下不参与），并列时保持注册顺序——而本包在宿主之后装配 ⇒ 不写就落末尾。故该席位只声明 `priority`。本 key 独占单元格，`priority` 的"同格遮蔽"语义在此不参与，只借其排序效果。
  - `settings.models.footer`（list）：账本按 `priority → order` 排完后，渲染器**又按 `order` 单键稳定重排一次** ⇒ 决定位置的是 `order`，故该席位只声明 `order`（`priority` 仅在同 `order` 时当平手判据，同值碰撞概率极低，不值得抢）。该席位宿主侧目前零其他注册方，改动无可见差别。
  - 退化风险：若宿主日后改掉某处排序，后果只是卡片回到末尾，不坏功能。
